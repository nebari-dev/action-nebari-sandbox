...
| `snapshot-mode` | <p>Platform profile only. Controls cluster snapshot/restore. - 'off' (default): normal flow. Creates a fresh cluster, runs NIC   install + deploy, waits for the platform to come up.</p> <ul> <li>'use': skips nic deploy. Restores a pre-baked cluster snapshot bundle from <code>snapshot-path</code> instead. The bundle must have been produced by a previous run with <code>snapshot-mode: create</code>.</li> <li>'create': runs the normal flow, then snapshots the resulting cluster into <code>snapshot-path</code> for later reuse. Intended for release pipelines that publish reusable snapshots.</li> </ul> | `false` | `off` |
| `snapshot-path` | <p>Filesystem path used by <code>snapshot-mode</code>. When <code>create</code>, the bundle is written here. When <code>use</code>, the bundle is read from here. Ignored when <code>snapshot-mode: off</code>. Path is created if missing.</p> | `false` | `/tmp/nebari-snapshot` |
<!-- action-docs-inputs action="action.yml" -->
