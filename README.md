...
| `in-cluster-dns` | <p>When 'true' (default), add a CoreDNS entry so the external Nebari hostnames (<code>*.&lt;domain&gt;</code>, e.g. <code>keycloak.nebari.local</code>) resolve to the gateway from INSIDE the cluster. Required for consumer pods doing server-side OIDC (see #69). Set 'false' if you manage in-cluster DNS yourself.</p> | `false` | `true` |
| `destroy` | <p>When 'true' (default), destroy the deployment (<code>nic destroy</code>) in the action's post step when the job ends, including on failure or cancellation. Set 'false' to leave the cluster running (e.g. to inspect it in a later step).</p> | `false` | `true` |
| `token` | <p>GitHub token used by the NIC installer for authenticated release/API calls (avoids the low unauthenticated rate limit).</p> | `false` | `${{ github.token }}` |
<!-- action-docs-inputs action="action.yml" -->
...
| name | description |
| --- | --- |
| `kubeconfig` | <p>Path to the kubeconfig file for the created kind cluster (also exported as KUBECONFIG).</p> |
...
| `keycloak-realm-admin-password` | <p>Keycloak admin password for the <em>nebari</em> realm. Provisioned asynchronously by NIC's realm-setup PostSync hook, so this may be empty if the secret has not materialized by the time extract-outputs.sh polls (see #27). When empty, read the <code>nebari-realm-admin-credentials</code> secret in the <code>keycloak</code> namespace after your own wait-for-realm step.</p> |
...
| `domain` | <p>The Nebari domain the platform was deployed with, e.g. <code>nebari.local</code>.</p> |
| `ca-cert-path` | <p>Path on the runner to the sandbox gateway's CA certificate, extracted from the <code>nebari-gateway-tls</code> secret. Use it for runner-side <code>curl --cacert</code>, or to build a ConfigMap/Secret your pods mount so they trust <code>https://&lt;domain&gt;</code>. The same CA is published in-cluster as ConfigMap <code>nebari-sandbox-ca</code> (key <code>ca.crt</code>) in <code>kube-public</code>. Empty if the gateway TLS secret could not be read (see #69).</p> |
<!-- action-docs-outputs action="action.yml" -->
